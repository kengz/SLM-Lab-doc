...
1. **Document major changes** in `CHANGELOG.md`
...
- Check GPU/CPU utilization: `dstack metrics <run-name>`
...
# DQN CartPole
uv run slm-lab slm_lab/spec/benchmark/dqn/dqn_cartpole.json dqn_boltzmann_cartpole train
# REINFORCE
...
### 6. Recognize When Stuck & Get Unstuck
When progress stalls, systematically diagnose and fix:
1. **Check GPU metrics** (`dstack metrics <run-name>`) - low GPU util means bottleneck in env stepping or config issue
2. **Review historical progress** in BENCHMARKS.md - compare current run to past successful runs on same/similar envs
3. **Look for patterns** - if same issue across runs, it's likely a framework bug or missing feature, not hyperparameters
4. **Research reference implementations** - CleanRL, SB3, rlzoo configs for the specific env
5. **Take a step back** - if stuck on same problem for multiple runs, stop and think about root cause vs symptoms
6. **Kill unpromising runs early** - don't wait for runs clearly failing; iterate faster with new approaches
